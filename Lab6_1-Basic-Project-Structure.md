...
Docker: สะดวก, สภาพแวดล้อมเหมือนกันทุกเครื่อง, ไม่กระทบระบบหลัก

Native: ต้องติดตั้งเอง, เสี่ยงปัญหาเวอร์ชันต่างกัน, ใช้ทรัพยากรน้อยกว่าเล็กน้อย

2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary

รัน container ที่มี ESP-IDF

โหลดโค้ดเข้า container

ใช้ idf.py build คอมไพล์และลิงก์ไฟล์

ได้ binary ในโฟลเดอร์ build

3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?

ควบคุมการ build project และ component

ระบุไฟล์ source, include paths

ทำงานเหมือน native ภายใน Docker

4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?

ป้องกันไฟล์ build/ชั่วคราว ไม่ให้เก็บใน Git

ลดขนาด repo และความสับสน

ปกป้องข้อมูลส่วนตัวหรือ config เฉพาะเครื่อง

5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?

หาย: ไฟล์ใน container ที่ไม่ mount เป็น volume

อยู่: โฟลเดอร์ที่ mount กับ host และ Docker volumes

6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system

Docker	

สะดวก, สภาพแวดล้อมเหมือนกัน	

รันใน container, ใช้ resource เพิ่ม	

ต้องจัดการข้อมูล persistence	

Native

ต้องติดตั้งเอง, อาจไม่เหมือนกัน

รันตรงบนเครื่อง, เร็วกว่าบ้าง

ข้อมูลอยู่ตรงเครื่องเลย
## 📋 ผลลัพธ์ที่คาดหวัง
