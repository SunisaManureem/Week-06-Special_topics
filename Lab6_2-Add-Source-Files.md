...
เพื่อแยกความรับผิดชอบของโค้ดให้ง่ายต่อการดูแลและพัฒนา ลดความซับซ้อน ช่วยให้ทีมงานทำงานพร้อมกันได้ และง่ายต่อการแก้ไขหรือเพิ่มฟีเจอร์ในอนาคต

2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?

ต้องเพิ่มชื่อไฟล์ source ใหม่ในคำสั่ง idf_component_register() หรือคำสั่งที่ระบุไฟล์ source เพื่อให้ CMake รวมไฟล์นั้นเข้าสู่การคอมไพล์

3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?

ใช้ประกาศฟังก์ชัน ตัวแปร และโครงสร้างข้อมูลให้ส่วนอื่น ๆ ของโปรแกรมรู้จัก ทำให้สามารถเรียกใช้ฟังก์ชันหรือข้อมูลจากไฟล์อื่นได้โดยไม่ต้องเขียนซ้ำ

4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?

ระบุโฟลเดอร์ที่เก็บไฟล์ header (.h) เพื่อให้คอมไพเลอร์รู้จักและสามารถหาไฟล์ header ที่ใช้ในโปรเจกต์ได้ถูกต้อง

5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?

ช่วยกำหนดไฟล์หรือโฟลเดอร์ที่ไม่ต้องการให้ Git ติดตาม เช่น ไฟล์บิลด์ ชั่วคราว หรือไฟล์คอนฟิกเฉพาะเครื่อง ลดความยุ่งเหยิงและขนาดของ repository

6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?

ช่วยให้การจัดการ LED ทำงานแบบแยกส่วนและพร้อมกันได้ เช่น การกะพริบ LED แบบไม่บล็อกการทำงานอื่น ๆ ทำให้ระบบมีประสิทธิภาพและตอบสนองเร็วขึ้น

7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?

การแยกโมดูล sensor, display, led เป็นไฟล์แยกช่วยให้โค้ดเป็นระเบียบ ดูแลรักษาง่าย ทดสอบแยกส่วนได้สะดวก และลดผลกระทบเมื่อแก้ไขส่วนใดส่วนหนึ่ง
...
- จำนวนไฟล์ source: source: 1
- ขนาด binary: 235,792 bytes
- การทำงาน: การอ่านค่าจากเซนเซอร์แล้วส่งข้อมูลไปให้ระบบประมวลผลหรือแสดงผล

**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: source: 3
- ขนาด binary: 164,816 bytes
- การทำงาน: การอ่านข้อมูลเซ็นเซอร์แล้วแสดงผลบนหน้าจอแบบวนลูป
